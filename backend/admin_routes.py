...
def list_payments(admin=Depends(get_current_admin)):
    ...
    try:
        res = supabase.table("payments").select(
            "id, client, payment_type, payment_status, montant, amount, moyen, numero, recipient_number, preuve, proof_url, statut, date, created_at, motif, review_reason, reviewer_id, reviewed_at"
        ).execute()
        return res.data or []
    except Exception as exc:
